...
vec4 GetDistance_Color(vec3 point)
{
	...
	{
		vec3 point_pos_vec = metaballs[i].pos.xyz - point;
		float point_pos_sqr = dot(point_pos_vec, point_pos_vec);
		float curr_charge = metaballs[i].color_charge.w / point_pos_sqr;
		// Cutoff
		float cutoff_radius = cutoff_radius_k * metaballs[i].color_charge.w;
		curr_charge *= int(point_pos_sqr < cutoff_radius * cutoff_radius);
		charge += curr_charge;
...
float GetDistance(vec3 point)
{
	...
	{
		vec3 point_pos_vec = metaballs[i].pos.xyz - point;
		charge += metaballs[i].color_charge.w / dot(point_pos_vec, point_pos_vec);
	}
...
{
	{ vec3(100.0, 100.0, 0.0), vec3(1.0) },
	{ vec3(-100.0, 0.0, 0.0), vec3(1.0) }
};

const vec3 ambient_light = vec3(0.4);
const float shininess = 128;
const float specular_k = 0.8;
const float diffuse_k = 0.8;
